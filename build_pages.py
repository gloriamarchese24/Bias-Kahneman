...
# 100% Clickable interactive HTML5 Component via st.components.v1.html
build_ab_page(
    ...
    "        b64_img = load_hydrant_b64()\n        if b64_img:\n            html_a = f'''<!DOCTYPE html><html><head><style>body {{ margin:0; padding:0; background:#111; font-family:sans-serif; text-align:center; color:white; }} .box {{ padding:1.2rem; background:#111; border-radius:20px; border:1px solid rgba(108,99,255,0.3); }} img {{ width:220px; border-radius:16px; filter:blur(24px); box-shadow:0 8px 32px rgba(0,0,0,0.5); transition: filter 0.1s linear; }} p {{ color:#6C63FF; font-weight:600; font-size:0.9rem; margin-top:12px; }} button {{ background:#6C63FF; color:white; border:none; padding:10px 20px; border-radius:10px; font-weight:700; font-size:0.95rem; cursor:pointer; margin-top:8px; box-shadow:0 4px 12px rgba(108,99,255,0.4); }} button:active {{ transform: scale(0.96); }}</style></head><body><div class=\"box\"><img id=\"imgA\" src=\"data:image/jpeg;base64,{b64_img}\" /><p id=\"txtA\">🎥 Messa a fuoco video fluida (8s)...</p><button id=\"btnA\" onclick=\"playA()\">▶️ Riproduci Animazione</button></div><script>function playA() {{ var img=document.getElementById('imgA'); var txt=document.getElementById('txtA'); var btn=document.getElementById('btnA'); var start=null; var dur=8000; txt.innerText=\"🎥 Messa a fuoco video fluida in corso (8s)...\"; btn.disabled=true; btn.style.opacity=\"0.6\"; function step(ts) {{ if(!start) start=ts; var progress=(ts-start)/dur; if(progress>1) progress=1; var curBlur=24-(progress*21); if(img) img.style.filter=\"blur(\"+curBlur+\"px)\"; if(progress<1) {{ window.requestAnimationFrame(step); }} else {{ txt.innerText=\"✅ Messa a fuoco completata!\"; btn.disabled=false; btn.style.opacity=\"1\"; }} }} window.requestAnimationFrame(step); }} setTimeout(playA, 300);</script></body></html>'''\n            components.html(html_a, height=360)\n        else:\n            st.markdown(\"\"\"<div style='text-align:center; padding:1.5rem; background:#111; border-radius:12px;'><div style='font-size:70px; filter:blur(9px); line-height:1;'>🧯🟡</div></div>\"\"\")\n        scelta = st.radio('Cosa ritieni rappresenti principalmente questa immagine?', ['A) Un Minion / Personaggio animato', 'B) Un Idrante antincendio / Estintore'], index=None, key='r1')\n",
    "        b64_img = load_hydrant_b64()\n        if b64_img:\n            html_b = f'''<!DOCTYPE html><html><head><style>body {{ margin:0; padding:0; background:#111; font-family:sans-serif; text-align:center; color:white; }} .box {{ padding:1.2rem; background:#111; border-radius:20px; border:1px solid rgba(255,166,0,0.3); }} img {{ width:220px; border-radius:16px; filter:blur(24px); box-shadow:0 8px 32px rgba(0,0,0,0.5); transition: filter 0.3s ease; }} p {{ color:#FFA600; font-weight:600; font-size:0.9rem; margin-top:12px; }} button {{ background:#FFA600; color:black; border:none; padding:10px 20px; border-radius:10px; font-weight:700; font-size:0.95rem; cursor:pointer; margin-top:8px; box-shadow:0 4px 12px rgba(255,166,0,0.4); }} button:active {{ transform: scale(0.96); }}</style></head><body><div class=\"box\"><img id=\"imgB\" src=\"data:image/jpeg;base64,{b64_img}\" /><p id=\"txtB\">🖼️ Frame 1 di 4 (Sfocatura iniziale 24px)...</p><button id=\"btnB\" onclick=\"playB()\">▶️ Riproduci Animazione</button></div><script>var timerId=null; function playB() {{ if(timerId) clearTimeout(timerId); var img=document.getElementById('imgB'); var txt=document.getElementById('txtB'); var btn=document.getElementById('btnB'); btn.disabled=true; btn.style.opacity=\"0.6\"; var frames=[{{b:24,l:\"🖼️ Frame 1 di 4 (Sfocatura iniziale 24px)\"}},{{b:16,l:\"🖼️ Frame 2 di 4 (Sfocatura marcata 16px)\"}},{{b:10,l:\"🖼️ Frame 3 di 4 (Sfocatura media 10px)\"}},{{b:3,l:\"🖼️ Frame 4 di 4 (Messa a fuoco finale 3px)\"}}]; var idx=0; function showNext() {{ if(img) img.style.filter=\"blur(\"+frames[idx].b+\"px)\"; if(txt) txt.innerText=frames[idx].l; idx++; if(idx<frames.length) {{ timerId=setTimeout(showNext, 2000); }} else {{ btn.disabled=false; btn.style.opacity=\"1\"; }} }} showNext(); }} setTimeout(playB, 300);</script></body></html>'''\n            components.html(html_b, height=360)\n        else:\n            st.markdown(\"\"\"<div style='text-align:center; padding:1.5rem; background:#111; border-radius:12px;'><div style='font-size:70px; filter:blur(5px); line-height:1;'>🧯🔴</div></div>\"\"\")\n        scelta = st.radio('Cosa ritieni rappresenti principalmente questa immagine?', ['A) Un Minion / Personaggio animato', 'B) Un Idrante antincendio / Estintore'], index=None, key='r2')\n",
    "",
...
build_ab_page(
    ...
    "        b64_img = load_hydrant_b64()\n        if b64_img:\n            html_a = f'''<!DOCTYPE html><html><head><style>body {{ margin:0; padding:0; background:#111; font-family:sans-serif; text-align:center; color:white; }} .box {{ padding:1.2rem; background:#111; border-radius:20px; border:1px solid rgba(108,99,255,0.3); }} img {{ width:220px; border-radius:16px; filter:blur(24px); box-shadow:0 8px 32px rgba(0,0,0,0.5); transition: filter 0.1s linear; }} p {{ color:#6C63FF; font-weight:600; font-size:0.9rem; margin-top:12px; }} button {{ background:#6C63FF; color:white; border:none; padding:10px 20px; border-radius:10px; font-weight:700; font-size:0.95rem; cursor:pointer; margin-top:8px; box-shadow:0 4px 12px rgba(108,99,255,0.4); }} button:active {{ transform: scale(0.96); }}</style></head><body><div class=\"box\"><img id=\"imgA\" src=\"data:image/jpeg;base64,{b64_img}\" /><p id=\"txtA\">🎥 Continuous fluid video unblurring in progress (8s)...</p><button id=\"btnA\" onclick=\"playA()\">▶️ Play Animation</button></div><script>function playA() {{ var img=document.getElementById('imgA'); var txt=document.getElementById('txtA'); var btn=document.getElementById('btnA'); var start=null; var dur=8000; txt.innerText=\"🎥 Continuous fluid video unblurring in progress (8s)...\"; btn.disabled=true; btn.style.opacity=\"0.6\"; function step(ts) {{ if(!start) start=ts; var progress=(ts-start)/dur; if(progress>1) progress=1; var curBlur=24-(progress*21); if(img) img.style.filter=\"blur(\"+curBlur+\"px)\"; if(progress<1) {{ window.requestAnimationFrame(step); }} else {{ txt.innerText=\"✅ Focus sequence completed!\"; btn.disabled=false; btn.style.opacity=\"1\"; }} }} window.requestAnimationFrame(step); }} setTimeout(playA, 300);</script></body></html>'''\n            components.html(html_a, height=360)\n        else:\n            st.markdown(\"\"\"<div style='text-align:center; padding:1.5rem; background:#111; border-radius:12px;'><div style='font-size:70px; filter:blur(9px); line-height:1;'>🧯🟡</div></div>\"\"\")\n        scelta = st.radio('What do you believe this image primarily depicts?', ['A) A Minion / Animated character', 'B) A Fire Hydrant / Fire Extinguisher'], index=None, key='r1')\n",
    "        b64_img = load_hydrant_b64()\n        if b64_img:\n            html_b = f'''<!DOCTYPE html><html><head><style>body {{ margin:0; padding:0; background:#111; font-family:sans-serif; text-align:center; color:white; }} .box {{ padding:1.2rem; background:#111; border-radius:20px; border:1px solid rgba(255,166,0,0.3); }} img {{ width:220px; border-radius:16px; filter:blur(24px); box-shadow:0 8px 32px rgba(0,0,0,0.5); transition: filter 0.3s ease; }} p {{ color:#FFA600; font-weight:600; font-size:0.9rem; margin-top:12px; }} button {{ background:#FFA600; color:black; border:none; padding:10px 20px; border-radius:10px; font-weight:700; font-size:0.95rem; cursor:pointer; margin-top:8px; box-shadow:0 4px 12px rgba(255,166,0,0.4); }} button:active {{ transform: scale(0.96); }}</style></head><body><div class=\"box\"><img id=\"imgB\" src=\"data:image/jpeg;base64,{b64_img}\" /><p id=\"txtB\">🖼️ Frame 1 of 4 (Initial blur 24px)...</p><button id=\"btnB\" onclick=\"playB()\">▶️ Play Animation</button></div><script>var timerId=null; function playB() {{ if(timerId) clearTimeout(timerId); var img=document.getElementById('imgB'); var txt=document.getElementById('txtB'); var btn=document.getElementById('btnB'); btn.disabled=true; btn.style.opacity=\"0.6\"; var frames=[{{b:24,l:\"🖼️ Frame 1 of 4 (Initial blur 24px)\"}},{{b:16,l:\"🖼️ Frame 2 of 4 (Heavy blur 16px)\"}},{{b:10,l:\"🖼️ Frame 3 of 4 (Medium blur 10px)\"}},{{b:3,l:\"🖼️ Frame 4 of 4 (Final focus 3px)\"}}]; var idx=0; function showNext() {{ if(img) img.style.filter=\"blur(\"+frames[idx].b+\"px)\"; if(txt) txt.innerText=frames[idx].l; idx++; if(idx<frames.length) {{ timerId=setTimeout(showNext, 2000); }} else {{ btn.disabled=false; btn.style.opacity=\"1\"; }} }} showNext(); }} setTimeout(playB, 300);</script></body></html>'''\n            components.html(html_b, height=360)\n        else:\n            st.markdown(\"\"\"<div style='text-align:center; padding:1.5rem; background:#111; border-radius:12px;'><div style='font-size:70px; filter:blur(5px); line-height:1;'>🧯🔴</div></div>\"\"\")\n        scelta = st.radio('What do you believe this image primarily depicts?', ['A) A Minion / Animated character', 'B) A Fire Hydrant / Fire Extinguisher'], index=None, key='r2')\n",
    "",
    "        v = 0 if 'Minion' in scelta else 1\n        supabase.table('Risposte').insert({'esperimento': NOME_ESPERIMENTO, 'gruppo': st.session_state.gruppo, 'valore': v}).execute()\n",
    lang="EN"
)
...
print("Rigenerazione completata con st.components.v1.html!")
